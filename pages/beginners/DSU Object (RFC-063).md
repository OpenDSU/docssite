...
The path in the current DSU where you want to create a new file.
...
<p align="justify">Description: Returns "undefined" stats if an Error occurs when calling BrickMapConstroller.stat(). Otherwise, stats are provided asynchronously to callback().</p>
...
Example 2: Writing / Reading a data string to a DSU object
...
<p align="justify">Description: Checks if a batch is currently in progress and synchronously returns true if a batch has been scheduled or is being executed. Otherwise, it returns false.</p>
Function beginBatch()

<p align="justify">Description: Schedules a new batch and disables automatic anchoring after each operation performed on this DSU instance. The callback function prevents anchoring after each DSU operation. From OpenDSU 2.0.0, the “batch mode” is mandatory anyway.

It throws an Error if batch processing is already in progress, as reported by batchInProgress().

This function returns an “batchID” as an identifier for a “batch”. The batch can be imagined as a “transaction” in the databases, but because we have a more file system approach, the metaphor is called “operation batching” or DSu “batch mode”</p>
### Function startOrAttachBatch(callback)

<p align="justify">Description: If a DSU is already in batch mode, invoking startOrAttachBatch() generates a new "virtual batch." This virtual batch has no side effects other than to specify that commitBatch() will execute successfully only if all current batches are closed.</p>

The callback also returns a batchId that can be used with cancelBatch pr commitBatch.
### Function cancelBatch(bacthID)

<p align="justify">Description: Cancel a  batch (remove the batch mode from the DSU) and remove any changes made. If multiple batches are open, it does nothing, and changes made in the current virtual batch could still be anchored. No perfect behaviour is possible; if this aspect is important, then only one “batch” should be allowed to be opened on an instance.</p>
...
<p align="justify">Description: Synchronously returns the number of unanchored changes if this DSU instance has pending changes to be anchored; otherwise, it returns false.</p>
...
The function that will be used as the anchoring event listener.
...
<p align="justify">Sets if the result is encoded or plain. True if you want the identifier in the ssi format (ssi:za:domain:specificStr:control:vn), false if you want it encoded in Base58.</p>

### Function getKeySSIAsObject(KeySSIType, callback)

<p align="justify">Description: Get an instance of the KeySSI corresponding to the DSU providing the keySSIType. If KeySSIType is "undefined" or not provided, the keySSI object for this DSU instance is lazily returned. Otherwise, the KeySSI object of this DSU instance is transformed into an object of type KeySSIType.

Returns an Error err if KeySSIType is not any of the registered KeySSI types or if this DSU instance's KeySSI object cannot be transformed to an object of type KeySSIType.</p>
Name
...
The type of the keySSI you are looking for

Leave empty to get seed/highest key. keyType must be derivable from the highest key.
...
<p align="justify">Description: Lazy wrapper for getKeySSIAsObject() for providing the identifier String of the computed KeySSI object to the callback function.
...
<p align="justify">Description: Retrieves the KeySSI used to mount the specified mountPoint by obtaining the archiveSSI identifier for the DSU instance mounted at the path mountPoint in this DSU object.

Returns an Error err if no DSU instance is mounted at the path mountPoint of this DSU object.</p>
...
The path to the mounted directory from which you want to obtain the identifier.
...
The path inside your DSU from which you want to retrieve the list of mounted dossiers.

Provides an Array with the list of all external DSU objects mounted under the specified folder mountPoint of this DSU instance.
...
<p align="justify">Description: Mounts the external DSU container identified by archiveSSI at the path mountingPoint of the DSU instance. The options parameter may encapsulate the flag persist (default: true) to save the updated manifest file after successfully mounting the external DSU object at mountingPoint to the manifest file of this DSU instance. Otherwise, the mounting point is stored only in a temporary variable of this DSU instance.

Returns an Error err if mountingPoint points to a non-empty directory or if another DSU instance is already mounted at mountingPoint.</p>
...
The path in your DSU where you want to mount the archive (the directory has to be empty).
...
The identifier (or the keySSI) of the archive you want to mount.
...
<p align="justify">Description: Removes the external DSU container mounted at mountingPoint. If the external DSU at mountingPoint has been mounted persistently, a new manifest file after removal of the externally mounted DSU container is saved to this DSU instance accordingly.

Returns an Error err if no mounted DSU is found at dsuPath.</p>
...
The path inside the DSU towards the archive you want to unmount.
...
The methods a DSU object provides for file handling are listed below, where all path arguments fsPath or dsuPath and the optional configuration flags in options.</p>

### Function addFile(fsPath, dsuPath, options, callback)

<p align="justify">Description: Copies one single file entry, specified by fsPath in the local file system, to the folder dsuPath of this DSU instance. Follows symbolic links in fsPath; if ignoreMounts is set to false (the default), it also loads externally mounted DSUs in dsuPath.
...
The path (from the current environment file system) towards the file you want to add to your DSU.
...
The path inside the DSU environment where you want to add the file.
...
<p align="justify">Description: Copies one or more files specified by the Array fsPaths from their paths in the local file system to the folder dsuPath of this DSU instance. Follows symbolic links in fsPath and loads externally mounted DSUs in dsuPath for default ignoreMounts: false.
...
Returns an Error err if source fsPaths or target dsuPath cannot be accessed.</p>
...
Array of the paths towards your files in the current environment file system.
...
The path inside the DSU environment where you want to add files.
...
<p align="justify">Description: Iterates the contents of a folder fsPath in the local file system and copies each entry to a folder dsuPath in the DSU instance. Follows symbolic links in fsPath and loads externally mounted DSUs in dsuPath if ignoreMounts is set to false (which is the default).
...
The path towards your files in the current environment.
...
<p align="justify">Description: Restores data stored in Brick objects under dsuPath to a file in the local file system as specified by fsPath. Configuration options may encapsulate the flag ignoreMounts (default: false).

Returns an Error err if an externally mounted DSU object in dsuPath cannot be loaded or if one of the Bricks objects for dsuPath cannot be accessed. Otherwise, data from Bricks objects is copied to the local file specified by fsPath.</p>
...
The path inside your current environment toward the file you want to extract.
...
The “path” where you want to “extract” your DAU  file.
...
<p align="justify">Description: Restores for all files under dsuPath the data stored in Brick objects to a local file system folder qualified by fsPath by lazy calls to extractFile(). Configuration options may encapsulate the flag ignoreMounts (default: false).
...
The path inside your current environment toward the folder you want to extract.
