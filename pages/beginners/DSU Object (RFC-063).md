...
Example 1: Code to instantiate a DSU object
...
Figure 1: The impact of global configuration options
...
Employed during write access to the DSU object. When enabled (true), the target of the operation is stored directly in the BrickMap rather than split across several Bricks (false) of the DSU. Especially for smaller files, setting this flag to “true” will reduce the time required for file access.
...
True enables the encryption for the Brick of the DSU container storing the target(s) of the corresponding operation.
...
When set to true, externally mounted DSUs are ignored when executing the corresponding operation. Default mode (false) takes external mounts into account.
...
Triggers the corresponding operation to be applied recursively to subfolders of the target(s).
...
Figure 2: Logical overview of file operation methods of DSU objects
...
The path inside the DSU from which you want to retrieve the archive.
...
The path in your DSU from which you want to retrieve the list of files.
...
The path in your DSU from which you want to retrieve the list of folders.
...
The path inside the current DSU from which you want to retrieve the files and folders.
...
The path of the original folder.
...
The path where the clone folder will be created.
...
The path where you want to create a folder inside your DSU environment.
...
The path inside the DSU toward the file you want to delete.
...
The path toward the file you want to rename.
...
The new name for the file or folder. The path should be the same except for the last segment.
...
The path toward the file you want to append data to (inside the DSU environment).
...
The data you want to append to your file.
...
The path inside you DSU for which you want to create a read stream.
...
The path toward a file inside your DSU that you want to read.
...
This function returns an “batchID” as an identifier for a “batch”. The batch can be imagined as a “transaction” in the databases, but because we have a more file system approach, the metaphor is called “operation batching” or DSu “batch mode”
...
A DSU object further employs the KeySSI object to initialise handlers for anchoring changes in the DSU file system to the underlying blockchain (e.g. BrickMapController and BrickMapStorageService).
...
Sets if the result is encoded or plain. True if you want the identifier in the ssi format (ssi:za:domain:specificStr:control:vn), false if you want it encoded in Base58.
...
Figure 3: Logical overview of file operation methods of DSU objects
